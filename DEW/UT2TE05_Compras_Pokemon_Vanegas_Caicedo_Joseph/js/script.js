...
function filterPokemons() {
  ...
  const pokemonCards = Array.from(document.getElementById("pokedex").children);

  pokemonCards.forEach(pokemonCard => {
    const matchesPower = filterByPower(pokemonCard, poderFilterValue);
    const matchesType = filterByType(pokemonCard, tipoFilterValue);
    const matchesWeight = filterByWeight(pokemonCard, pesoFilterValue);

    pokemonCard.style.display = (matchesPower && matchesType && matchesWeight) ? 'block' : 'none';
  });
}

function filterByPower(card, value) {
  if (!value) return true;
  const powers = card.querySelectorAll('.attack');
  return Array.from(powers).some(power => power.textContent.toLowerCase().includes(value));
}

function filterByType(card, value) {
  if (!value) return true;
  const types = card.querySelectorAll('.type');
  return Array.from(types).some(type => type.textContent.toLowerCase().includes(value));
}

function filterByWeight(card, value) {
  if (!value) return true;
  const weightText = card.querySelector('.weight').textContent.toLowerCase();
  const weightValue = parseFloat(value);
  const pokemonWeight = parseFloat(weightText.match(/weight:\s*(\d+\.?\d*)/)[1]);
  return pokemonWeight === weightValue;
}
  ...
  if (userDoc.exists()) {
    ...
    for (const pokemon of desireList) {
      try {
        ...
        <br>
        <img class="front" src="${pokemonData.sprites.front_default}" alt="Front sprite"> 
        <img class="front" src="${pokemonData.sprites.front_shiny}" alt="Shiny Front sprite"> 
        <img class="back" src="${pokemonData.sprites.back_default}" alt="Back sprite"> 
        <img class="shiny-back" src="${pokemonData.sprites.back_shiny}" alt="Shiny Back sprite"> 
        <br>
        <div class="type ${singleTypeClass}" style="background-color: ${tipo1Color};">
          ${tipo1}
        </div>
...
const purchasedButton = document.getElementById('btn_ver_comprados');

purchasedButton.addEventListener('click', async () => {
  const userId = auth.currentUser   ? auth.currentUser  .uid : null; 
  if (!userId) {
      alert("Please log in to view your purchased Pokémon.");
      return;
  }

  // Hide the Pokedex and Cart if they are visible
  document.getElementById("pokedex").style.display = 'none';
  ...
  if (userDoc.exists()) {
      const userData = userDoc.data();
      const purchasedList = userData.purchasedPokemons || [];

      const purchasedListContainer = document.getElementById("purchasedListContainer");
      purchasedListContainer.innerHTML = ""; 
      purchasedListContainer.style.display = "grid"; 

      for (const pokemon of purchasedList) {
          try {
              const response = await fetch(`https://pokeapi.co/api/v2/pokemon/${pokemon.id}`);
              const pokemonData = await response.json();

              // Convert Firestore Timestamp to Date
              const purchaseDate = pokemon.purchaseDate.toDate(); 
              const formattedDateTime = purchaseDate.toLocaleString(); 
              ...
              <div class="card" id="${pokemon.id}">
                  ...
                  <div class="purchase-date">
                      Purchased on: ${formattedDateTime} <!-- Display the formatted date and time -->
                  </div>
              </div>
              `;
              purchasedListContainer.appendChild(pokemonCard);
          } catch (error) {
              console.error("Error fetching Pokémon data:", error);
          }
      }
